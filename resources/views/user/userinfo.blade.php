...
@section('main-content')
    ...
    <div class="row info" id="base-info">
        ...
        <div class="col-md-3 col-sm-4">
            <div class="thumbnail">
                <img src="/images{{ isset($user_detail->pic_path) ? $user_detail->pic_path : '/common/mrtx.jpg' }}" alt="...">
    ...
    <script>
        var cert = new Vue({
            ...
            data: {
                edit: false,
                edit_btn: "编辑",
                real_name: '{{ isset($user_detail->real_name) ? $user_detail->real_name : '' }}',
                id_card: '{{ isset($user_detail->id_card) ? $user_detail->id_card : '' }}',
                passport: '{{ isset($user_detail->passport) ? $user_detail->passport : '' }}',
            },
            methods: {
                showEdit: function () {
                    if (this.edit) {
        ...
        var other = new Vue({
            ...
            data: {
                edit: false,
                edit_btn: "编辑",
                sex: '{{ isset($user_detail) ? $user_detail->sex : ''}}',
                birth: '{{ isset($user_detail) ?  $user_detail->birth : '' }}',
                country: '{{ isset($user_detail) ? $user_detail->country : ''}}',
                province: '{{ isset($user_detail) ?  $user_detail->province : ''}}',
                city: '{{ isset($user_detail) ? $user_detail->city : ''}}',
                education: '{{ isset($user_detail) ? $user_detail->education : '' }}',
                job: '{{ isset($user_detail) ? $user_detail->job : ''}}'
            },
            methods: {
                showEdit: function () {
                    if (this.edit) {
